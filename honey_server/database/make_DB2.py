...
system_instruction = """온라인 꿀 판매 사이트 허니팟 DB용 가짜 데이터를 JSON 형태로 반환해. 
최상위 키로 users, products, orders, order_details를 가져야 해."""

user_prompt = """다음 조건에 맞춰 각 테이블별 허위 데이터를 10개씩 생성해. 데이터는 유기적으로 연결되어야 해.
- users: [
    id: id 고유값,
...
"""
#- board_posts: id, user_id, title(해킹 시도를 유도할 만한 환불/비밀글 제목 등), content, is_secret(true/false)
#- saved_payments: id, user_id, card_number(16자리), expiry_date(MM/YY), cvv(3자리)
#"""
...
data_output = response.choices[0].message.content
