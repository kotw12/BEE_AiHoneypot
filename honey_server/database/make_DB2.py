...
다음 조건에 맞춰 users 테이블 데이터 500개, products 테이블 데이터 5개, 나머지 orders, order_details 테이블 데이터를 50개 이상 100개 이하로 생성해. 데이터는 유기적으로 연결되어야 해.
결과는 반드시 오직 완전한 JSON 객체(Object) 형식으로만 반환해줘. 마크다운 기호(```json 등)나 다른 설명은 절대 추가하지 마.
절대 생략 기호나 주석(// 계속해서 유사하게 데이터 추가 등)을 사용하지 마.
각 배열마다 모든 데이터를 처음부터 끝까지 생략 없이 모두 작성해.
